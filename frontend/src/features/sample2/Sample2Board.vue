  ...
  <div class="sample-board">
    ...
    <section class="table-section">
      ...
      <div v-else-if="samples.length === 0" class="empty-state">
        <p>등록된 샘플2가 없습니다.</p>
        <p class="empty-hint">위 폼을 사용해서 첫 번째 샘플2를 추가해보세요!</p>
      </div>
 ...
 * 샘플2 데이터의 생성, 조회, 수정, 삭제 기능을 제공합니다.
 */
export default {
  name: 'Sample2Board',
  ...
  methods: {
    ...
    async loadSamples() {
      try {
        ...
        console.error('샘플2 목록 조회 실패:', error);
        this.showError('샘플2 목록을 불러오는데 실패했습니다.');
      } finally {
    ...
    async handleSubmit() {
      ...
      if (this.form.id) {
        ...
        console.error('샘플2 저장 실패:', error);
        const action = this.form.id ? '수정' : '생성';
        this.showError(`샘플2 ${action}에 실패했습니다.`);
      } finally {
        this.isLoading = false;
      }
    },
    ...
    async confirmDelete(sample) {
      const confirmMessage = `"${sample.name}" 샘플2를 정말 삭제하시겠습니까?`;
